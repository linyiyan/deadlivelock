...
class locksetHelper = 
	...
	method constr_acqstr context lockset acq = 
		let  acqstr = String.concat "," (Ss.elements lockset)
		in let acqstr = String.concat ";" [context.fname;acqstr;"mutex_lock";acq]
		in acqstr
		...
		Call(_,Lval(lvar),AddrOf(arg)::lst,_)::xil ->
			...
			begin		
				...
				if opr = "pthread_mutex_lock" && not (Ss.is_empty lockset) then
					begin	
						let acqstr = self#constr_acqstr context lockset acq in
					...
					begin
						Ss.iter 
							(fun hold -> 
								...
								in 
								Ss.iter (fun acq_str -> locksetg#add_str_edge acq_str (self#constr_acqstr context lockset acq) ; ()) acq_strs
							) 
						lockset ;
					locksetg;
					end
				...
				begin
					if opr = "pthread_mutex_lock" && not (Ss.is_empty lockset) then	
						...
						if Sm.mem acq acq_cache then 
							begin
							acq_cache <- Sm.add acq (Ss.add (self#constr_acqstr context lockset acq) (Sm.find acq acq_cache)) acq_cache;
							(* printf "%s --> " acq;
							Ss.iter (fun c-> printf "%s | " c) (Sm.find acq acq_cache); *)
							end
						else 
							let ns = Ss.add (self#constr_acqstr context lockset acq) (Ss.empty) in 
								...
								fun hold -> 
									begin
										if Sm.mem hold hold_cache then
											hold_cache <- Sm.add hold (Ss.add (self#constr_acqstr context lockset acq) (Sm.find hold hold_cache)) hold_cache
										else 
											let ns = Ss.add (self#constr_acqstr context lockset acq) (Ss.empty) in 
